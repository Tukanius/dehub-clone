...
import 'package:dehub/src/product_module/screens/inventory_reference/unit/add_unit_sheet.dart';
...
class InventoryUnit extends StatefulWidget {
  static const routeName = '/InventoryUnit';
  const InventoryUnit({super.key});

  @override
  State<InventoryUnit> createState() => _InventoryUnitState();
}

class _InventoryUnitState extends State<InventoryUnit> with AfterLayoutMixin {
  int page = 1;
  int limit = 20;
  User user = User();
  bool isLoading = true;
  ...
  list(page, limit) async {
    ...
    distributor = await InventoryApi()
        .unitList(ResultArguments(offset: offset, filter: filter));
    groupMaker();
    setState(() {
      isLoading = false;
    });
  ...
  update(InventoryGoods data) {
    updateSheet(context, updateClick: () {
      if (Permission().check(user, "ERP_REF_UNIT", boolean: 'isEdit')) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton:
          Permission().check(user, "ERP_REF_UNIT", boolean: 'isCreate')
              ? FloatingActionButton(
                  onPressed: () {
                    showModalBottomSheet(
                      context: context,
