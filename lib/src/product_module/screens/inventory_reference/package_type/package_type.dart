...
class PackageType extends StatefulWidget {
  static const routeName = '/PackageType';
...
class _PackageTypeState extends State<PackageType> with AfterLayoutMixin {
  int page = 1;
  int limit = 20;
  User user = User();
  bool isLoading = true;
  ...
  list(page, limit) async {
    ...
    distributor = await InventoryApi()
        .packageTypeList(ResultArguments(offset: offset, filter: filter));
    groupMaker();
    setState(() {
      isLoading = false;
    });
