...
class InActiveTypes extends StatefulWidget {
  static const routeName = '/InActiveTypes';
  const InActiveTypes({super.key});

  @override
  State<InActiveTypes> createState() => _InActiveTypesState();
}

class _InActiveTypesState extends State<InActiveTypes> with AfterLayoutMixin {
  int page = 1;
  int limit = 20;
  User user = User();
  bool isLoading = true;
  ...
  list(page, limit) async {
    ...
    distributor = await InventoryApi()
        .inactiveTypeList(ResultArguments(offset: offset, filter: filter));
    groupMaker();
    setState(() {
      isLoading = false;
    });
